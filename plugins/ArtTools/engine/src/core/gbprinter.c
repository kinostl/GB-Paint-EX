...
uint8_t printer_print_cgb_tile(uint8_t tiledata[16], uint8_t tile_attrs) {
    ...
    const uint8_t h_flip = tile_attrs & (1 << 5);
    const uint8_t v_flip = tile_attrs & (1 << 6);
    
    if(h_flip){
        for(i = 0; i < 16; i++) {
            tiledata[i] = reverse(tiledata[i]);
        }
    }
    
    if(v_flip){
        for(i = 15; i >= 0; i--) {
            printer_send_receive(tiledata[i]);
        }
    }else{
        for(i = 0; i < 16; i++) {
            printer_send_receive(tiledata[i]);
...
uint8_t gbprinter_print_overlay(uint8_t start, uint8_t rows, uint8_t margins) BANKED {
    uint8_t tile_data[16], error, packets, pkt_count = 0, tile_attrs = 0;
    ...
    for (uint8_t y = packets << 1; y != 0; y--, map += 0x20) {
        for (uint8_t x = 0, *row = map; x != 20; x++, row++) {
            ...
            #ifdef CGB
            if (_is_CGB) {
                if (printer_print_cgb_tile(tile_data, tile_attrs)) pkt_count++;
            } else{
